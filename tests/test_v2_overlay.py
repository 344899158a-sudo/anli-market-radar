...
class V2OverlayTests(unittest.TestCase):
    ...
    def test_overlays_v2_without_rewriting_verified_snapshot(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual((site / "index.html").read_text(encoding="utf-8"), "legacy shell")
            self.assertIn("ANLI 2.0", (site / "playbooks.html").read_text(encoding="utf-8"))
            self.assertTrue((site / "v2_styles.css").is_file())
            self.assertTrue((site / "v2_app.js").is_file())
            self.assertNotIn("今日纪律", (site / "v2_app.js").read_text(encoding="utf-8"))
            self.assertNotIn("disciplinePanel", (site / "v2_app.js").read_text(encoding="utf-8"))
