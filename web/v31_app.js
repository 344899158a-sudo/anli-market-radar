(() => {
  ...
  function renderToday(data) {
    ...
    return `<section class="view ${state.currentView === "today" ? "active" : ""}" data-view-panel="today">
      ${qualityBanner(data)}
      <div class="page-head"><div><span class="eyebrow">TODAY'S UNIFIED COMMAND</span><h1>今日决策</h1></div><p>3.1 在3.0统一决策之上增加组合风险和不可改写样本；所有状态仍只用于研究，不会自动下单。</p></div>
      <section class="command-card">
        <div class="command-top"><div><span class="eyebrow">MARKET COMMAND</span><h2>${e(command.verdict || "等待统一决策")}</h2><p class="command-copy">${e(command.posture || "等待市场、行业、剧本与证据链完整。")}</p></div><div class="risk-orb"><div><strong>${fmt(command.risk_cap_pct, 0)}%</strong><span>市场风险预算上限</span></div></div></div>
    ...
    return `<section class="view ${state.currentView === "market" ? "active" : ""}" data-view-panel="market">
      ...
      <div class="market-grid">
        <section class="panel market-hero"><div class="panel-header"><div><span class="eyebrow">MARKET GATE</span><h2>${e(gate.label || overview.regime_label || "市场待确认")}</h2><p>${e(gate.regime_label || overview.position_label || "")}</p></div>${statusBadge(gate.state || "CAUTION", gate.state || "CAUTION")}</div>
          <div class="score-line"><div class="score-ring"><span><strong>${integer(gate.market_score ?? overview.score)}</strong><small>趋势环境强度 / 100</small></span></div><div class="market-verdict"><h2>${e(gate.position_label || overview.position_label || "位置待确认")}</h2><p>${e(gate.action || overview.action || "等待大盘与风险闸门完成。")}</p><small class="score-warning">${e(data.model_labels?.market_score?.warning || "该分数不等于适合交易概率。")}</small></div></div>
          <div class="evidence-grid">
            ${overviewTile("风险上限", `${fmt(gate.allocation_cap_pct, 0)}%`, "3.1继续采用2.0保守口径")}
      ...
      <div class="section-grid">
        ...
        <section class="panel"><div class="panel-header"><div><span class="eyebrow">VERSION DEPTH</span><h2>1.0、2.0、3.0 全部保留</h2><p>3.1只增加验证与组合风控，不改写任何旧版历史结论。</p></div></div>
          <div class="constraint-list"><div class="constraint-item"><strong>3.1 当前页</strong><p>统一命令、组合边界、影子观察与后验验证。</p></div><div class="constraint-item"><strong>3.0 统一系统</strong><p>保留整合完成时的原始统一API和页面。</p></div></div>
          <div class="detail-actions"><a href="./v3.html">打开 3.0 原版</a><a class="secondary" href="./v1.html">打开 1.0 全景</a></div>
        </section>
      </div>
    </section>`;
  }
  ...
  function renderOpportunities(data) {
    return `<section class="view ${state.currentView === "opportunities" ? "active" : ""}" data-view-panel="opportunities">
      ${qualityBanner(data)}
      <div class="page-head"><div><span class="eyebrow">OPPORTUNITIES + PLAYBOOKS</span><h1>机会与剧本</h1></div><p>每只股票只保留一个当前剧本；1.0分数作为证据参考，3.1不改变3.0以2.0剧本入口为准的状态口径。</p></div>
      ${playbookCards(data)}
  ...
  function portfolioEventRadar(data) {
    const radar = data.portfolio_event_radar || {};
    const events = Array.isArray(radar.events) ? radar.events : [];
    if (radar.state === "NO_SELECTION" || !events.length) {
      return `<section class="panel"><div class="panel-header"><div><span class="eyebrow">PORTFOLIO EVENT RADAR</span><h2>本周持仓与行业风险</h2><p>${e(radar.label || "尚未匹配持仓事件")}</p></div></div><div class="empty-state">${e(radar.next_action || "在本机标记持仓后，系统会优先列出直接、行业关联和全市场风险事件。")}</div></section>`;
    }
    return `<section class="panel"><div class="panel-header"><div><span class="eyebrow">PORTFOLIO EVENT RADAR · ${e(radar.policy_version)}</span><h2>本周持仓与行业风险</h2><p>${e((radar.focus_symbols || []).join("、"))} · ${e((radar.focus_sectors || []).join("、"))}</p></div>${statusBadge(radar.state, radar.label)}</div><div class="constraint-list">${events.map(event => `<div class="constraint-item"><strong>${e(event.relevance_label)} · ${e(event.title)}</strong><p>${e(event.why_relevant)}<br>${shortTime(event.at_cn || event.at)} · ${e(event.verification || "待核验")}${event.source_url ? `<br><a href="${e(event.source_url)}" target="_blank" rel="noopener noreferrer">核对官方来源</a>` : ""}</p></div>`).join("")}</div><p class="market-limitations">${e(radar.next_action)}</p></section>`;
  }
  ...
  function renderEvents(data) {
    return `<section class="view ${state.currentView === "events" ? "active" : ""}" data-view-panel="events">
      ...
      ${portfolioEventRadar(data)}
      <section class="panel" style="margin-top:14px"><div class="panel-header"><div><span class="eyebrow">INDUSTRY EVENT FILTER</span><h2>未来四周行业风险</h2><p>公司财报会同时按直接标的、所属行业和关联产业链标记。</p></div></div>${eventSectorPicker(data)}${weekCards(data)}</section>
  ...
  function validationHorizonCards(validation) {
    const rows = Array.isArray(validation.horizons) ? validation.horizons : [];
    if (!rows.length) return `<div class="empty-state">尚未形成未来交易日结果；系统不会用当前价格冒充未来表现。</div>`;
    return `<div class="validation-horizons">${rows.map(row => `<article>
      <span>${integer(row.horizon_sessions)}个交易日后</span>
      <strong>${row.sample_sufficient ? `${fmt(row.positive_rate_pct, 1)}%` : "样本积累中"}</strong>
      <small>样本 ${integer(row.sample_count)} · 中位回报 ${signed(row.median_return_pct)}</small>
    </article>`).join("")}</div>`;
  }

  function portfolioPositionRows(risk) {
    const rows = Array.isArray(risk.positions) ? risk.positions : [];
    if (!rows.length) return `<div class="empty-state">尚无完整持仓明细</div>`;
    return `<div class="portfolio-table"><div class="portfolio-head"><span>持仓</span><span>市值占比</span><span>开放风险</span><span>止损</span></div>${rows.map(row => `<div class="portfolio-row"><span><strong>${e(row.symbol)}</strong><small>${e(row.sector)}</small></span><span>${fmt(row.market_value_pct, 2)}%</span><span>${fmt(row.open_risk_pct, 2)}%</span><span>${money(row.stop_price)}</span></div>`).join("")}</div>`;
  }

  function riskChecks(risk) {
    const rows = Array.isArray(risk.checks) ? risk.checks : [];
    return `<div class="risk-checks">${rows.map(item => `<article>
      <header>${statusBadge(item.status, item.status)}<strong>${e(item.label)}</strong></header>
      <p>${e(item.evidence)}</p><small>${e(item.next_condition)}</small>
    </article>`).join("") || `<div class="empty-state">组合风险证据尚未建立</div>`}</div>`;
  }

  function renderValidation(data) {
    const validation = data.validation || {};
    const risk = data.portfolio_risk || {};
    const primaryProgress = `${integer(validation.primary_sample_count, 0)}/${integer(validation.minimum_sample_count, 30)}`;
    return `<section class="view ${state.currentView === "validation" ? "active" : ""}" data-view-panel="validation">
      ${qualityBanner(data)}
      <div class="page-head"><div><span class="eyebrow">VALIDATION + PORTFOLIO RISK</span><h1>验证与风险闭环</h1></div><p>每一份决策快照和后验结果只追加、不改写；组合资料用于风险计算，不会连接券商或生成订单。</p></div>
      <div class="validation-metrics">
        <article><span>不可改写快照</span><strong>${integer(validation.snapshot_count, 0)}</strong><small>相同市场状态自动去重</small></article>
        <article><span>逐股观察</span><strong>${integer(validation.observation_count, 0)}</strong><small>保留当时状态、价格和剧本</small></article>
        <article><span>影子设置</span><strong>${integer(validation.shadow_setup_count, 0)}</strong><small>只在研究就绪时建立</small></article>
        <article><span>${integer(validation.primary_horizon_sessions, 5)}日样本门槛</span><strong>${e(primaryProgress)}</strong><small>${e(validation.label || "正在积累样本")}</small></article>
      </div>
      <div class="section-grid">
        <section class="panel risk-panel"><div class="panel-header"><div><span class="eyebrow">PORTFOLIO GATE</span><h2>${e(risk.label || "组合风险待确认")}</h2><p>${e(risk.next_action || "先补齐组合资料。")}</p></div>${statusBadge(risk.state, risk.state)}</div>
          <div class="risk-summary"><div><span>开放风险</span><strong>${fmt(risk.open_risk_pct, 2)}%</strong></div><div><span>总持仓市值</span><strong>${fmt(risk.market_value_pct, 2)}%</strong></div><div><span>明细覆盖</span><strong>${fmt(risk.coverage_pct, 0)}%</strong></div><div><span>持仓数量</span><strong>${integer(risk.position_count, 0)}</strong></div></div>
          ${portfolioPositionRows(risk)}
          ${publicMode
            ? `<div class="constraint-item validation-note"><strong>公开只读版</strong><p>账户净值、持仓数量、成本与止损不会上传或公开；请在本机3.1中维护组合风险资料。</p></div>`
            : `<button class="primary-button" data-edit-portfolio>录入或更新组合明细</button>`}
        </section>
        <section class="panel"><div class="panel-header"><div><span class="eyebrow">OUTCOME ANALYSIS</span><h2>${e(validation.label || "正在积累不可改写样本")}</h2><p>${e(validation.warning || "后验观察不等于真实成交收益。")}</p></div>${statusBadge(validation.sample_sufficient ? "PASS" : "CAUTION", validation.status)}</div>
          ${validationHorizonCards(validation)}
          <div class="constraint-item validation-note"><strong>当前结论</strong><p>${validation.sample_sufficient ? "主观察周期达到最低样本门槛，但仍需检查成本、分市场环境和样本外稳定性。" : "最低样本门槛未达到，页面不会把短期结果称为胜率或已验证优势。"}</p></div>
        </section>
      </div>
      <section class="panel" style="margin-top:14px"><div class="panel-header"><div><span class="eyebrow">ACCOUNT PROTECTION</span><h2>1.0账户纪律已重新接入</h2><p>回撤熔断优先于缺失数据和一般风险超限；任何资料不完整都按不扩大风险处理。</p></div></div>${riskChecks(risk)}</section>
    </section>`;
  }

  function renderAll() {
    if (!state.data) return;
    app.innerHTML = renderToday(state.data) + renderMarket(state.data) + renderOpportunities(state.data) + renderEvents(state.data) + renderValidation(state.data);
    syncNavigation();
  }
  ...
  function setView(view) {
    if (!["today", "market", "opportunities", "events", "validation"].includes(view)) return;
  ...
  function openSymbol(symbol) {
    ...
    drawerBody.innerHTML = `<section class="drawer-hero"><div class="drawer-symbol"><div><span class="eyebrow">${e(row.sector)}</span><h2>${e(row.symbol)}</h2><p>${e(row.name)}</p></div><div class="drawer-price"><strong>${money(row.price)}</strong><span class="${changeClass(row.day_change_pct)}">${signed(row.day_change_pct)}</span></div></div><div class="drawer-badges">${statusBadge(row.research_state?.code, row.research_state?.label)}${statusBadge(row.entry?.state, row.entry?.label)}<span class="status-badge status-PASS">证据 ${integer(row.evidence_completion_pct)}%</span>${row.holding ? `<span class="status-badge status-CAUTION">我的持仓</span>` : ""}</div></section>
      ...
      <section class="detail-section"><h3>AI / 基本面复核</h3><p>${e(row.ai_analysis?.summary || "当前没有新鲜且可验证的AI基本面结论；AI不会直接改变3.1研究状态。")}</p></section>
  ...
  function openPortfolioEditor() {
    if (publicMode) {
      showToast("公开版不接收账户或持仓资料，请在本机3.1中录入");
      return;
    }
    const risk = state.data?.portfolio_risk || {};
    const input = risk.input || { account: {}, positions: [] };
    const account = input.account || {};
    const existing = new Map((input.positions || []).map(item => [item.symbol, item]));
    const holdings = [...new Set([...(risk.marked_holdings || []), ...existing.keys()])].sort();
    drawerEyebrow.textContent = "ANLI 3.1 · PORTFOLIO INPUT";
    drawerTitle.textContent = "组合风险资料";
    drawerBody.innerHTML = `<form id="portfolioForm" class="portfolio-form">
      <section class="drawer-hero"><span class="eyebrow">LOCAL + VERSIONED</span><h2>只保存本机风险快照</h2><p>不会连接券商，不会下单；每次保存都会新增版本，历史记录不可改写。</p></section>
      <section class="detail-section"><h3>账户风险状态</h3><div class="form-grid">
        <label><span>账户净值</span><input name="equity" type="number" min="0.01" step="0.01" required value="${e(account.equity || "")}" placeholder="例如 100000"></label>
        <label><span>日回撤 %</span><input name="daily_drawdown_pct" type="number" min="0" step="0.01" value="${e(account.daily_drawdown_pct || "0")}"></label>
        <label><span>周回撤 %</span><input name="weekly_drawdown_pct" type="number" min="0" step="0.01" value="${e(account.weekly_drawdown_pct || "0")}"></label>
        <label><span>月回撤 %</span><input name="monthly_drawdown_pct" type="number" min="0" step="0.01" value="${e(account.monthly_drawdown_pct || "0")}"></label>
        <label><span>连续亏损次数</span><input name="consecutive_losses" type="number" min="0" step="1" value="${e(account.consecutive_losses || "0")}"></label>
      </div></section>
      <section class="detail-section"><h3>已标记持仓</h3><p class="form-help">数量、平均成本和止损价全部必填；止损价必须低于当前市场价。</p><div class="position-inputs">${holdings.map(symbol => { const row=existing.get(symbol)||{}; return `<div class="portfolio-position-input" data-position-symbol="${e(symbol)}"><strong>${e(symbol)}</strong><label><span>数量</span><input data-field="quantity" type="number" min="0.0001" step="any" required value="${e(row.quantity || "")}"></label><label><span>平均成本</span><input data-field="average_cost" type="number" min="0.0001" step="any" required value="${e(row.average_cost || "")}"></label><label><span>止损价</span><input data-field="stop_price" type="number" min="0.0001" step="any" required value="${e(row.stop_price || "")}"></label></div>`; }).join("") || `<div class="empty-state">当前没有标记持仓；可以保存空仓账户状态，或先在1.0中标记持仓。</div>`}</div></section>
      <div class="form-actions"><button class="primary-button" type="submit">确认并保存新版本</button><button class="secondary-button" type="button" data-close-drawer>取消</button></div>
    </form>`;
    drawer.classList.add("open");
    drawer.setAttribute("aria-hidden", "false");
    document.body.style.overflow = "hidden";
  ...
  async function savePortfolio(form) {
    if (publicMode) {
      showToast("公开版为只读快照，组合资料不会上传");
      return;
    }
    const formData = new FormData(form);
    const positions = [...form.querySelectorAll("[data-position-symbol]")].map(row => ({
      symbol: row.dataset.positionSymbol,
      quantity: row.querySelector('[data-field="quantity"]').value,
      average_cost: row.querySelector('[data-field="average_cost"]').value,
      stop_price: row.querySelector('[data-field="stop_price"]').value,
    }));
    const payload = {
      account: {
        equity: formData.get("equity"),
        daily_drawdown_pct: formData.get("daily_drawdown_pct") || "0",
        weekly_drawdown_pct: formData.get("weekly_drawdown_pct") || "0",
        monthly_drawdown_pct: formData.get("monthly_drawdown_pct") || "0",
        consecutive_losses: Number(formData.get("consecutive_losses") || 0),
      },
      positions,
    };
    ...
    try {
      const idempotencyKey = `portfolio-${Date.now()}-${Math.random().toString(16).slice(2)}`;
      await jsonFetch("/api/v3.1/portfolio", {
        method: "POST",
        headers: { "Content-Type": "application/json", "X-ANLI-Confirm": "portfolio-update", "Idempotency-Key": idempotencyKey },
        body: JSON.stringify(payload),
      });
      await load();
      closeDrawer();
      setView("validation");
      showToast("组合资料已作为新版本保存，历史版本保持不变");
    } catch (error) {
  ...
  document.addEventListener("click", event => {
    ...
    if (event.target.closest("[data-edit-portfolio]")) openPortfolioEditor();
